...
```{r echo=FALSE}
tbl1 <- tb3 %>% subset(Age == "I alt") %>% group_by(Gender) %>% summarise(mortality_per_mio = mean(mortality_rate*1E6), CVpct = CVpct(mortality_rate)) %>% ungroup()

kable(tbl1)

kable(subset(tb3, Age == "I alt") %>% group_by(Gender) %>% summarise(population = mean(population)) %>% ungroup())
